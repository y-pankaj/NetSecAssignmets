x = int(input("Input 1 to change the text by 1 bit and 2 to change key by 1 bit\n"))
...
def reshape_text(text,blocklen):
    padding = len(text)-len(text)%blocklen
...
class des():
    def __init__(self, blocklen, rounds):
        ...
        global SHIFT, CP_1,E,PI,PI_1,P,CP_2
...
#             print("done")
            g, d = nsplit(block, self.blocklen//2) #g(LEFT), d(RIGHT)
...
#         print(len(CP_1))
        split_part = self.blocklen - (self.blocklen)/8

        # 28 was the param here
    ...
    def addPadding(self):#Add padding to the datas using PKCS5 spec.
        pad_len = self.blocklen - (len(self.text) % self.blocklen)
...
#     key = "secret_k"
#     text= "Hello wo"
#     text = "0000000000000000000000000000000000001111000000000000000000000000"
...
#     text = ""
    # key= "0000001100101101001001100010001110000110000011100011001010001010"
    key = "11100011001010010010001100101110101001101101000011010111001100000011001011010010011000100011100001100000111000110010100010100001"
    blocklen = 128
    # print(len(text))
    ...
    if len(text)%blocklen ==0:
        padlen=0
    elif(len(text)<blocklen):
        padlen = blocklen - len(text) +1
    else:
        padlen = len(text)-len(text)%blocklen
...
# print(len(text))
    ...
    #     print("len(r)",len(r))
        r2 = d.decrypt(key,r)
    #     print(r2)
    #     print("padlen %s",(padlen))
    #     r2 = r2[:-padlen]
        # print("Ciphered: %r" % r)
        # print("Deciphered: ", r2)
        if x==1 :
            if text[0]=="0":
                text2="1"+text[1:blocklen]
            else:
                text2="0"+text[1:blocklen]
        else :
            text2=text
        if x==2:
            if key[0]=="0":
                key2="1"+key[1:blocklen]
            else:
                key2="0"+key[1:blocklen]
        else :
            key2=key
        # print(len(text))
        if len(text2)%blocklen ==0:
            padlen=0
        elif(len(text2)<blocklen):
            padlen = blocklen - len(text2) +1
        else:
            padlen = len(text2)-len(text2)%blocklen
        for i in range(1,padlen):
            text2 = text2 +"0"
        d = des(blocklen,rounds)
    #     print(text)
    #     print(len(text))
        r_22 = d.encrypt(key2,text2)
    #     print(r)
    #     print("len(r)",len(r))
        r2 = d.decrypt(key2,r_22)
        ...
        for j in range(blocklen):
            if r[j] != r_22[j]:
                cnt+=1
        D[k]=cnt
        # print("Number of rounds %s   Hamming distance %s", (k,cnt))
