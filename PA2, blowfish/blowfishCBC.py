...
# funtion to split the input data into blocks
...
# encrypt function in blowfish
def encryptFn(data):
    # shifing the left block by 32 bits to perform operations
    L = data>>32
    R = data & 0xffffffff
    for i in range(0,16):
        # performing round funtion
        L = p[i]^L
        L1 = functionF(L)
        R = R^functionF(L1)
        L,R = swap(L,R)
...
# the function F as described in blow fish
def functionF(L):
    temp = s[0][L >> 24]
...
if __name__ == "__main__":
    ...
    # taking a random nonce to encrypt first block of plaintext
    nonce = random.randint(10**15,10**16)
    # splitting the plaintext in to blocks
    plaintextInBlocks = splitIntoBlocks(plaintext)
    # list to contain blocks of ciphertexts
    ciphertexts = []
    # iterating blocks of plaintexts and encrypting them
    for i in range(len(plaintextInBlocks)):
        if i==0:
            # using nonce to encrypt the first block
            ciphertexts.append(encryptFn(plaintextInBlocks[i]^nonce))
        else:
            # using the previous ciphertext block to encrypt the current block of plaintext
            ciphertexts.append(encryptFn(plaintextInBlocks[i]^ciphertexts[-1]))
    ...
    # printing the decrypted blocks of message
    for i in range(len(decryptedPlaintext)):
        print("P{} = {}".format(i+1,decryptedPlaintext[i]))
        recoveredPT = recoveredPT+str(decryptedPlaintext[i])
    # printing the final recovered/decrypted plaintext
    print("Recovered plaintext is: ",recoveredPT)